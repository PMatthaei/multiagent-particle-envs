...
class WorldObservationTestCases(unittest.TestCase):
    ...
    def test_a_observes_b(self):
        #   vis     health    rel x-pos   rel y-pos  rel dist  unit bits
        range = self.a.sight_range
        a_obs_of_b = np.array([1.0, 1.0, 1.0 / range, 0.0 / range, 1.0 / range, 0, 0, 0])
        np.testing.assert_array_equal(self.world.obs[self.a.id][self.b.id], a_obs_of_b)

    def test_b_observes_a_with_different_sight_range(self):
        range = self.b.sight_range
        b_obs_of_a = np.array([1.0, 1.0, -1.0 / range, 0.0 / range, 1.0 / range, 0, 0, 0])
