...
class EnvironmentSpacesTestCases(unittest.TestCase):
    ...
    def test_get_info(self):
        dummy = {
            "state_shape": self.agents_n * self.state_per_agent_dim,
            "obs_shape": self.obs_n,
            "n_actions": self.agents_n + self.movement_dim + self.noop,
            "n_agents": self.policy_agents,
            "episode_limit": 60
        }
        info = self.env.get_env_info()
        self.assertEqual(info, dummy)
